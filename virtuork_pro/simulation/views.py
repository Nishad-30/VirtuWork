...
def initiate_ai_logic(request, simulation_id):
    try:
        ...
        project_info = thinker.generate_project(sim.role_title, sim.education)
...
def submit_task(request):
    if request.method == "POST":
        ...
        # BUG FIX 1: Prevent submission for tasks that don't require it
        if not task.requires_submission:
            return JsonResponse({'status': 'error', 'message': 'This task does not require a submission.'})

        # BUG FIX 2: Enforce the 3-attempt limit 
        if task.attempts >= 3:
            return JsonResponse({'status': 'error', 'message': 'Maximum attempts reached for this task.'})
        
        task.attempts += 1
        task.save()

        # Retrieve/Generate solution template
        ...
        if passed:
            ...
            manager.adjust_difficulty(simulation, score) # [cite: 85]
            msg_text = f"Great work! Your submission for {task.title} passed. {feedback}"
        elif not passed and not is_last_attempt:
            msg_text = f"I've reviewed your submission for {task.title}. Score: {score}%. Feedback: {feedback}. You have {3 - task.attempts} attempts left."
        else:
            # 3RD FAILED ATTEMPT: Force completion and provide the solution 
            task.is_completed = True
            msg_text = f"I've reviewed your third attempt for {task.title}. Score: {score}%. Let's move on so you don't get stuck. Here is the expected logic: \n\n{simulation.expected_output_template}"
            simulation.expected_output_template = "" # Reset for next task
            simulation.save()
...
def send_message_ajax(request):
    ...
    conv = get_object_or_404(Conversation, id=conv_id, simulation__user=request.user)
    sim = conv.simulation
    ss_agent = SoftSkillsAgent()
    new_scores = ss_agent.analyze_message(text)
    if new_scores:
        ss_agent.update_heatmap(sim, new_scores)
    ...
    return JsonResponse({
        ...
        'new_task': {
            ...
            'expected_file_extension': current_task.expected_file_extension,
...
def end_simulation_report(request, simulation_id):
    """
    Finalizes the simulation and generates the comprehensive AI performance report.
    """
    # 1. Fetch the simulation and ensure it belongs to the user
    simulation = get_object_or_404(Simulation, id=simulation_id, user=request.user)

    # 2. Mark the simulation as completed if it's still ongoing
    if simulation.status != 'completed':
        simulation.status = 'completed'
        simulation.save()

    # 3. Trigger the PerformanceAgent to generate the final report
    # This analyzes chat history, technical scores, and soft skills [cite: 21, 23, 81]
    perf_agent = PerformanceAgent()
    report_data = perf_agent.generate_final_report(simulation)

    # 4. Save the generated data into the database for future viewing
    simulation.final_report_data = report_data
    simulation.save()

    # 5. Render the final report template
    context = {
        'sim': simulation,
        'report': report_data,
        'tasks': simulation.tasks.all().order_by('order')
    }
